...
BioMart::Formatter::ALL

=head1 SYNOPSIS

The ALL Formatter returns data formatted into a ALL table
for a BioMart query's ResultTable

=head1 DESCRIPTION

When given a BioMart::ResultTable containing the results of 
a BioMart::Query the ALL Formatter will return ALL formatted tabular 
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XALL 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html>
<head>
  <title>%s</title>
...
#       }
       ...
       if ($reference){
           ...
           $result_seen = 1 if ($$row[$$attribute_positions[$i]] =~ /^[ACGT]$/ );
       }
...
# table- and document-closing tags (to keep ALL valid).
sub getFooterText {   
    return q{
</table>
</body>

...
BioMart::Formatter::SAME

=head1 SYNOPSIS

The SAME Formatter returns data formatted into a SAME table
for a BioMart query's ResultTable

=head1 DESCRIPTION

When given a BioMart::ResultTable containing the results of 
a BioMart::Query the SAME Formatter will return SAME formatted tabular 
...
package BioMart::Formatter::SAME;
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XSAME 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html>
<head>
  <title>%s</title>
...
sub nextRow {
   ...
   for (my $i = 0; $i < @{$attribute_positions}; $i++){
       ...
       if (($i != @{$attribute_positions} - 1) && ($$row[$$attribute_positions[$i]] ne $reference) && $$row[$$attribute_positions[$i]] && $reference){
          $$row[$$attribute_positions[$i]] = '' if ($$row[$$attribute_positions[$i]] =~ /^[ACGT]$/);
       }
...
# table- and document-closing tags (to keep SAME valid).
sub getFooterText {   
    return q{
</table>
</body>
